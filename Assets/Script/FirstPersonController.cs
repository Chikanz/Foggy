...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        {
            if (!m_CharacterController.isGrounded || !playFootsteps)
            {
                return;
            }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.tag.Equals("FogZone"))
            {
                StartCoroutine(FadeAndReset(9.5f, 0.5f));
                Debug.Log("start");
            }
        ...
        private IEnumerator FadeAndReset(float duration, float endDensity)
        {
            if (isFading) yield break; //Stop fading twice
            isFading = true;
            ...
            var fog = GetComponentInChildren<SSMS.SSMSGlobalFog>();
            var startDensity = fog.fogDensity;
            float elapsed = 0;
            ...
            transform.GetChild(0).GetChild(0).gameObject.SetActive(true); //activate black
            yield return new WaitForSeconds(4.20f); //wait for laugh

            //Move back to start
            transform.position = Checkpoint;
            fog.fogDensity = startDensity;
            m_WalkSpeed = 4;
            m_RunSpeed = 5;
            isFading = false;
            playFootsteps = true;
